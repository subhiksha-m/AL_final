...
class ActiveLabeler():
    ...
    def strategy(self, name, query, N, strategy_params):
        ...
        if name == 'gaussian':
            ...
            def get_samples(image_list, confidences, number_of_samples):
                ...
                picked_samples = np.random.choice(image_list,
                                                    replace=False,
    ...
    def get_images_to_label_offline(self, model, sampling_strat, sample_size,
                                    strategy_params, device, path, data_type):
        """
        ...
        path -- path of images (pass none if the data_type is embeddings)
        ...
        if data_type == "embeddings":
            ...
            with torch.no_grad():
                ...
                subset = self.strategy(sampling_strat, model_predictions,
                                        sample_size, strategy_params)
            ...
            with torch.no_grad():
                ...
                strategy_params = {}
                strategy_params['image_list']= image_paths
                subset = self.strategy(sampling_strat, unlabled_probablites,
                                        sample_size, strategy_params)
                strategy_embeddings = []
                if sampling_strat=="gaussian":
                    strategy_images = subset
